...
if (isset($_SESSION['user_email1'])) {
    $email = $_SESSION['user_email1'];
    $usql = mysqli_query($con, "SELECT * FROM admins WHERE admin_email = '$email'");
    $admindata = mysqli_fetch_array($usql);

    $studentcount = mysqli_query($con, "SELECT * FROM student");
    $studentsTotal = count(mysqli_fetch_all($studentcount));

    $date = date('Y-m-d');

    $complaintSql = mysqli_query($con, "SELECT * FROM complain WHERE comp_time >= '$date 00:00:00' AND comp_time <= '$date 23:59:59' AND comp_status = 'PENDING'");
    $totalComplaints = count(mysqli_fetch_all($complaintSql));
}
?>
    ...
    <div class="flex items-center gap-5">
        ...
        <div class="profile z-10" x-data="dropdown" @click.outside="open = false">
            <button type="button" class="flex items-center gap-2.5" @click="toggle()">
                <div class="text-start">
                    <div class="flex items-center gap-1">
                        <span class="hidden xl:block text-sm font-semibold"><?= $admindata['admin_name'] ?></span>
                    ...
                    <span class="hidden xl:block text-xs text-lightgray">Admin</span>
                </div>
            </button>
            <ul x-show="open" x-transition="" x-transition.duration.300ms="" style="display: none;">
                ...
                <li>
                    <a href="logout.php" class="!text-danger flex items-center gap-2">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
